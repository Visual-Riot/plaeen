    ...
    try {
        ...
        (session: { admin_id: string; dates: any[] }) => {
          ...
          dates.forEach((date) => {
            ...
            hours.forEach((hour: number) => {
              ...
              if (admin_id === user_id) {
                newDayHours[day][hour] = "6"; // Current user is admin, state 6
              } else {
                newDayHours[day][hour] = "7"; // Another user is admin, state 7
              }
          ...
          userSchedule.availability.forEach((availability: Availability) => {
            ...
            availability.hours.forEach(({ hour, state }) => {
              if (!newDayHours[day][hour]) {
                newDayHours[day][hour] = "0"; // Initialize state to 0 if not already set
              }

              if (state === "2" || state === "3") {
                // If the member is available, set state to "1"
      ...
      for (const day in newDayHours) {
        for (const hour in newDayHours[day]) {
          let availableCount = newDayHours[day][hour] === "1" ? 1 : 0;
